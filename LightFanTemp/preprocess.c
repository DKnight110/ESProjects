...
int test1()
{
 ...
 uint32_t arry[42];
 int i;
 memset(arry, 0, sizeof(arry)/sizeof((arry)[0]));

 for (int i26 = ((0) * (18 / 3)); i26 < (((0) + 1) * (18 / 3)); i26 ++) { arry[i26] = 0x12345678; };

 for(i = ((0) * (18 / 3)); i < (((0) + 1) * (18 / 3)); i++) {
  if (arry[i] != 0x12345678) {
   fprintf(
# 30 "unit_tests.c" 3 4
          stderr
# 30 "unit_tests.c"
                ,"Error: LED at pos %d doesn't match color %d\n", i, 0x12345678);
   return -1;
  }
 }

 for(i = (((0) + 1) * (18 / 3)); i < (((7) + 1) * (18 / 3)); i++) {
  if (arry[i] != 0) {
   fprintf(
# 37 "unit_tests.c" 3 4
          stderr
# 37 "unit_tests.c"
                ,"Error: LED at pos %d doesn't match initial value %d\n", i, 0);
   return -2;
  }
 }
 return 0;
