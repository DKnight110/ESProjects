...
#define CHECK_WATER_INTERVAL      120UL /* seconds */
...
//  {
    ...
    //SERIAL_PRINT("Calibrating "); SERIAL_PRINTLN(x);
    //SERIAL_PRINT("Avg = "); SERIAL_PRINTLN(water_avg);
    //delay(50);
  //}
  //water_avg /= 8;
...
void loop(void)
{
  ...
  if (watchdog_counter == INT_TO_SLEEP_TIME(WAKEUP_BATT_LEVEL_INT)) {
    ...
    //if (check_water()) {
    if ((check_water == 0) && (watchdog_counter == INT_TO_SLEEP_TIME(CHECK_WATER_INTERVAL))) {
      /* 
       * Don't check every AT_TINY_SLEEP intervals if water is still present; rather allow
       * for about 2 mins to pass till you check again. This is to guard against missing
       * the first message and limiting the time.
       */
      check_water = 1;
    }
    if ((check_water == 1) && (digitalRead(WATER_SENSOR_PIN) == LOW)) {
      //cli();
      //PCMSK &= ~_BV(SWITCH_PIN_INTERRUPT);
      check_water = 0;
  ...
  SERIAL_PRINT("check_water = "); SERIAL_PRINTLN(check_water);
